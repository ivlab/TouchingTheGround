...
plot.title = textGrob("Time", gp=gpar(fontsize=14, fontface='bold'))
...
range.error.plot = ggplot(summary.error[summary.error$Task == 'range',], aes(x=Task, y=mean, fill=Modality)) +
  ...
  theme(legend.position='none') +
  scale_fill_brewer(type="qual", palette="Paired") +
  scale_y_continuous('Mean error (% difference from correct range)', limits=c(0, 0.15), labels = function(i) paste0(round(100 * i), "%")) +
  xlab(NULL) +
  geom_bar(position=position_dodge(.9), stat="identity") +
  geom_errorbar(position=position_dodge(.9), width=.25, aes(ymin=mean-ci.ca, ymax=mean+ci.ca))
...
compare.error.plot = ggplot(df.error.compare.summary, aes(x='compare*', y=mean, fill=Modality)) +
  ...
  theme(axis.title.x = element_text(size=10, color = '#555555')) +
  xlab('compare*') +
  # ylim(0, 0.3) +
  scale_y_continuous('% Responses Incorrect', limits=c(0, 0.3), labels = function(i) paste0(round(100 * i), "%")) +
  scale_fill_brewer(type="qual", palette="Paired") +
  geom_bar(position=position_dodge(.9), stat="identity") +
...
x.label = textGrob("Task")
plot.title = textGrob("Error", gp=gpar(fontsize=14, fontface='bold'))
...
plot.title = textGrob("Confidence", gp=gpar(fontsize=14, fontface='bold'))
...
calculate those differences and their descriptive statistics.
...
diff.time.plot = ggplot(diff.time,
  ...
  theme_bw() +
  theme(legend.position = "none", plot.title = element_text(hjust = 0.5), aspect.ratio = 0.4) +
