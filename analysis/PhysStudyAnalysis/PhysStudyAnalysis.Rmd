...
# https://stackoverflow.com/a/28845828
...
df.coding = read.csv(paste(data.dir, 'questionnaire-normalized.csv', sep='/'))
...
# Normalized error for advect is reporting % of HALF circle away from correct answer.
summary.error$mean.360 = summary.error$mean * 180
...
summary.error$ci.ca.low.360 = summary.error$ci.ca.low * 180
summary.error$ci.ca.high.360 = summary.error$ci.ca.high * 180
...
This section contains secondary analysis, for example demographic info

### Load data

```{r}
df.time.demographic = data.frame(df.time)
df.time.demographic$Error[df.time.demographic$Error == 0] <- 0.0001
df.time.demographic$logError = log(df.time.demographic$Error)
df.time.demographic$VR.Usage2 = 'Have not used'
df.time.demographic$Topo.Usage2 = 'Have not used'
df.time.demographic$VideoGames2 = '< 2 hr/day'
```


### Clean data / replace with binary values

```{r}
df.time.demographic[
  df.time.demographic$VR.Usage == '1 to 5 times' |
  df.time.demographic$VR.Usage == '6 to 20 times' |
  df.time.demographic$VR.Usage == 'More than 20 times'
,]$VR.Usage2 = 'Have used'

df.time.demographic[
  df.time.demographic$Topo.Usage == '1 to 5 times' |
    df.time.demographic$Topo.Usage == '6 to 20 times' |
    df.time.demographic$Topo.Usage == 'More than 20 times'
  ,]$Topo.Usage2 = 'Have used'

df.time.demographic[
  df.time.demographic$VideoGames == '2hr to 6hr a day' |
    df.time.demographic$VideoGames == 'More than 6hr a day'
  ,]$VideoGames2 = '>= 2 hr/day'
```


### Summarize secondary values

```{r}
vr.usage.summary = summaryWithin(
  data=df.time.demographic,
  measure.var="TimeOnTask",
  within.vars=c('Modality'),
  between.vars=c('VR.Usage2'),
  subject.var="ParticipantID",
  conf.interval=0.95
)

topo.usage.summary = summaryWithin(
  data=df.time.demographic,
  measure.var="TimeOnTask",
  within.vars=c('Modality'),
  between.vars=c('Topo.Usage2'),
  subject.var="ParticipantID",
  conf.interval=0.95
)

kable(vr.usage.summary, label = 'VR Usage Statistics')
kable(topo.usage.summary, label = 'VR Usage Statistics')
```
...
theme.raw.common = theme(
    ...
    line = element_line(linewidth = plot.line.width),
...
time.plot = ggplot(summary.time, aes(x=Task, y=mean, fill=Modality)) +
  ...
  geom_errorbar(color="black", position=position_dodge(0.9), width=0.25, linewidth = plot.line.width, aes(ymin=mean-ci.ca, ymax=mean+ci.ca)) +
...
### Advect Error ####
# Normalized error for advect is reporting % of HALF circle away from correct answer.
yaxis.advect.values = c(0/180, 15/180, 30/180, 45/180)
advect.error.plot = ggplot(summary.error[summary.error$Task == 'advect',], aes(x=Task, y=mean, fill=Modality)) +
  theme_bw() +
  theme.raw.common +
  # theme(aspect.ratio = 6) +
  scale_fill_brewer(type="qual", palette="Paired") +
  scale_y_continuous('Mean error (degrees)', limits=c(0, 45/180), breaks=yaxis.advect.values, labels = function(i) paste0(round(180 * i), "°")) +
  xlab(NULL) +
  geom_bar(position=position_dodge(.9), stat="identity") +
  geom_errorbar(position=position_dodge(.9), width=.25, linewidth = plot.line.width, aes(ymin=mean-ci.ca, ymax=mean+ci.ca))
...
#   xlab('compare*') +
  ...
  scale_fill_brewer(type="qual", palette="Paired") +
  geom_bar(position=position_dodge(.9), stat="identity") +
  geom_errorbar(position=position_dodge(.9), width=.25, linewidth=plot.line.width, aes(ymin=mean-ci.basic, ymax=mean+ci.basic))
...
fig.raw.final = plot_grid(
    ...
    ncol = 3,
    labels = "AUTO",
    label_size = plot.font.size,
    rel_widths = c(0.8, 1.6, 0.8)
...
# width set from tvcg full page width, ~514.9pt ~~ 7.15in, saving 0.5in for the legend
ggsave('./analysis/figures/raw-results.pdf', plot = fig.raw.final, width = 6.65, units = "in")
...
theme.diff.common = theme(
  legend.position = "none",
  plot.title = element_text(hjust = 0.5, size = plot.font.size, face = "bold"),
...
diff.time.plot = ggplot(diff.time,
  ...
  geom_errorbar(position = position_dodge(0.9), width = 0.25, linewidth = plot.line.width, aes(xmin = ci.low, xmax = ci.high)) +
...
```{r}
dir.create('./analysis/figures')
# width set from tvcg column width, ~251pt ~~ 3.48in
ggsave('./analysis/figures/diff-results.pdf', plot = fig.diff.final, width = 3.48, units = "in")
```


## Qualitative / Open-Ended Feedback Analysis

Generate tables mentioning the most common positives, negatives, and strategies
people used in the study

### Clean data
```{r}
df.coding.2d = df.coding[df.coding$Modality == '2D',]
df.coding.vr = df.coding[df.coding$Modality == 'VR',]
df.coding.phys = df.coding[df.coding$Modality == 'Physical',]
```

### Positives
```{r}
positives.2d = count(c(df.coding.2d$PositiveFactor1, df.coding.2d$PositiveFactor2, df.coding.2d$PositiveFactor3))
positives.2d = positives.2d[positives.2d$x != '',]
write.excel(positives.2d[order(positives.2d$freq, decreasing = TRUE),], col.names = FALSE)
```
```{r}
positives.vr = count(c(df.coding.vr$PositiveFactor1, df.coding.vr$PositiveFactor2, df.coding.vr$PositiveFactor3))
positives.vr = positives.vr[positives.vr$x != '',]
write.excel(positives.vr[order(positives.vr$freq, decreasing = TRUE),], col.names = FALSE)
```
```{r}
positives.phys = count(c(df.coding.phys$PositiveFactor1, df.coding.phys$PositiveFactor2, df.coding.phys$PositiveFactor3))
positives.phys = positives.phys[positives.phys$x != '',]
write.excel(positives.phys[order(positives.phys$freq, decreasing = TRUE),], col.names = FALSE)
```

### Negatives
```{r}
negatives.2d = count(c(df.coding.2d$NegativeFactor1, df.coding.2d$NegativeFactor2, df.coding.2d$NegativeFactor3))
negatives.2d = negatives.2d[negatives.2d$x != '',]
write.excel(negatives.2d[order(negatives.2d$freq, decreasing = TRUE),], col.names = FALSE)
```
```{r}
negatives.vr = count(c(df.coding.vr$NegativeFactor1, df.coding.vr$NegativeFactor2, df.coding.vr$NegativeFactor3))
negatives.vr = negatives.vr[negatives.vr$x != '',]
write.excel(negatives.vr[order(negatives.vr$freq, decreasing = TRUE),], col.names = FALSE)
```
```{r}
negatives.phys = count(c(df.coding.phys$NegativeFactor1, df.coding.phys$NegativeFactor2, df.coding.phys$NegativeFactor3))
negatives.phys = negatives.phys[negatives.phys$x != '',]
write.excel(negatives.phys[order(negatives.phys$freq, decreasing = TRUE),], col.names = FALSE)

```

### Strategies
```{r}
strategies.2d = count(c(df.coding.2d$Strategy1, df.coding.2d$Strategy2, df.coding.2d$Strategy3))
strategies.2d = strategies.2d[strategies.2d$x != '',]
write.excel(strategies.2d[order(strategies.2d$freq, decreasing = TRUE),], col.names = FALSE)
```
```{r}
strategies.vr = count(c(df.coding.vr$Strategy1, df.coding.vr$Strategy2, df.coding.vr$Strategy3))
strategies.vr = strategies.vr[strategies.vr$x != '',]
write.excel(strategies.vr[order(strategies.vr$freq, decreasing = TRUE),], col.names = FALSE)

```
```{r}
strategies.phys = count(c(df.coding.phys$Strategy1, df.coding.phys$Strategy2, df.coding.phys$Strategy3))
strategies.phys = strategies.phys[strategies.phys$x != '',]
write.excel(strategies.phys[order(strategies.phys$freq, decreasing = TRUE),], col.names = FALSE)
```